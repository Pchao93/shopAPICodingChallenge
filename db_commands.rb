...
def find_user_by_id(id)
  (con.exec "SELECT * FROM users WHERE id = #{id}")[0]
end

def find_user_by_email(email)
  (con.exec "SELECT * FROM users WHERE email_address = #{email}")[0]
end
...
def find_license_key_by_user_id(user_id)
  (con.exec "SELECT * FROM license_keys WHERE user_id = #{user_id}")[0]
end

def find_license_key_by_id(id)
  (con.exec "SELECT * FROM license_keys WHERE id = #{id}")[0]
end
...
def find_order_by_id(id)
  (con.exec "SELECT * FROM orders WHERE id = #{id}")[0]

end
def find_order_by_customer_id(customer_id)
  (con.exec "SELECT * FROM orders WHERE customer_id = #{customer_id}")[0]

end
def find_order_by_shop_id(shop_id)
  (con.exec "SELECT * FROM orders WHERE shop_id = #{shop_id}")[0]
end
